<template>
	<view class="container">
		<view class="score-wrap">
			...
			<view class="desc">{{ `共${result.question_count}题，满分${result.total_score}分` }}</view>
		</view>
		...
		<view class="number-wrap">
			<view class="correct-wrap">
				...
				<view class="number-item">
					<view class="number">{{ result.question_count - result.correct_count }}</view>
					<view class="desc">错误</view>
				</view>
				<view class="number-item">
					<view class="number">{{ result.correct_count && (result.correct_count / result.question_count *
						100).toFixed(1) }}%</view>
	...
	methods: {
		...
		getTime() {
			try {
				...
				const hour = Math.floor(time / 1000 / 60 / 60).toString().padStart(2, '0')
				const minute = Math.floor(time / 1000 / 60 % 60).toString().padStart(2, '0')
