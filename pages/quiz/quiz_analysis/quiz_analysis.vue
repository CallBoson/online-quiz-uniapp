		...
		<view class="questions-wrap">
			...
			<view class="question-list">
				<view class="question-item" v-for="(item,index) in questions" :key="index">
					...
					<view class="score">{{ `(此题${item.question_score}分 得分${item.score}分)` }}</view>
					
					<view class="options-wrap">
						<view class="option-item" :class="{ correct: option.isAnswer }" v-for="(option, index) in item.options" :key="index">
							<view class="option-title">{{ String.fromCharCode(65 + index) }}.</view>
					...
					<view class="answer-wrap">
						...
						<view class="my-answer" :class="item.correct ? 'correct' : 'wrong'">我的答案：{{ item.answer }}</view>
			...
			uni.post('/quiz/result', {
				...
				this.questions = Array.from(result.questions, (item) => {
					...
					if (item.type == 1) {
						...
						obj.correct_answer = JSON.parse(item.answer_single).find(item => item.isAnswer).content
					} else if (item.type == 2) {
						obj.options = JSON.parse(item.answer_multiple)
						obj.correct_answer = JSON.parse(item.answer_multiple).filter(item => item.isAnswer).map(item => item.content).join('、')
					} else if (item.type == 3) {
						obj.options = [
							{ content: '正确', isAnswer: JSON.parse(item.answer_judge).answer ? true : false },
							{ content: '错误', isAnswer: JSON.parse(item.answer_judge).answer ? false : true }
						]
						obj.correct_answer = JSON.parse(item.answer_judge).answer ? '正确' : '错误'
					} else if (item.type == 4) {
						let str = ''
					...
					obj.question_score = result.each_score[item.type == 1 ? 'single' : item.type == 2 ? 'multiple' : item.type == 3 ? 'judge' : item.type == 4 ? 'fill' : 'short']
