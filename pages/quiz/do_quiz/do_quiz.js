    ...
    methods: {
        ...
        startClock() {
            ...
            clock = setInterval(() => {
                ...
                this.clock = `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`
              }, 500)
        },
        // 停止计时
        stopClock() {
        ...
        selectMultiple(index) {
            this.question_list[this.current].answer_multiple[index].isChecked = !this.question_list[this.current].answer_multiple[index].isChecked
        },
        // 选择判断题答案
        selectJudge(index) {
        ...
        submit() {
            uni.showModal({
                ...
                success: res => {
                    if (res.confirm) {
                        this.stopClock()
                        uni.post('/quiz/submit', {
                            record_id: this.quiz.record_id,
                            question_list: this.question_list
                        }).then(res => {
                            uni.redirectTo({
                                url: '/pages/quiz/quiz_result/quiz_result?id=' + this.quiz.record_id
                            })
                        })
                    }
                }
