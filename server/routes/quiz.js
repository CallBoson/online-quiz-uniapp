...
const {
  ...
  check,
  validateQuestion,
} = require("../handler/validateForm.js");
...
router.post(
  ...
  [
    ...
    check("title")
      .not()
      .isEmpty()
      .withMessage("答题标题不能为空")
      .isLength({ min: 2, max: 20 })
      .withMessage("答题标题长度为2-20个字符"),
    check("description")
      .isLength({ min: 0, max: 300 })
      .withMessage("答题描述最长为300个字符"),
    check("each_score")
      .custom((value) => {
        const values = {
          single: value?.single,
  ...
  async (req, res) => {
    ...
    const createdQuiz = await Quiz.create({
      ...
      date_range:
        data.date_range instanceof Array
          ? JSON.stringify(data.date_range)
          : null,
...
router.post(
  ...
  async (req, res) => {
    ...
    if (quizRecord) {
      ...
      if (
        date_range &&
        new Date().getTime() >= new Date(date_range[1]).getTime()
      ) {
        // 更改答题记录状态为已完成
        quizRecord.status = 3;
    ...
    res.ok({
      question_list: Array.from(questionList, (q) => {
        return {
          ...
          answer_single:
            q.type == 1
              ? Array.from(JSON.parse(q.answer_single), (a) => {
                  return {
                    content: a.content,
                    isChecked: false,
                  };
          ...
          answer_multiple:
            q.type == 2
              ? Array.from(JSON.parse(q.answer_multiple), (a) => {
                  return {
                    content: a.content,
                    isChecked: false,
                  };
          ...
          answer_fill:
            q.type == 4
              ? new Array(JSON.parse(q.answer_fill).length).fill("")
              : null,
          answer_judge:
            q.type == 3
              ? [
...
router.post(
  ...
  [
    ...
    check("question_list")
      .isArray()
      .withMessage("答题列表必须为数组")
      .not()
      .isEmpty()
      .withMessage("答题列表不能为空"),
  ],
  validationResult,
  async (req, res) => {
    const data = {
    ...
    for (let i = 0; i < questionList.length; i++) {
      ...
      if (userQuestion.type == 1) {
        ...
        answer_analysis[question.id] = {
          ...
          correct: isCorrect,
          score: isCorrect
            ? Number(JSON.parse(quizRecord.Quiz.each_score).single)
            : 0,
        };
        ...
        const userAnswers = userQuestion.answer_multiple.filter(
          (a) => a.isChecked
        ); // 用户选项
        // 判断正确选项的内容是否与用户选项的内容相同
        if (
          correctAnswers.every((a) =>
            userAnswers.some((b) => b.content == a.content)
          ) &&
        ...
        answer_analysis[question.id] = {
          ...
          correct: isCorrect,
          score: isCorrect
            ? Number(JSON.parse(quizRecord.Quiz.each_score).multiple)
            : 0,
        };
        ...
        const userAnswer = userQuestion.answer_judge.find(
          (a) => a.isChecked
        ) || { content: "" }; // 用户答案
        if (
          (userAnswer.content === "正确" && answer) ||
          (userAnswer.content === "错误" && !answer)
        ) {
        ...
        answer_analysis[question.id] = {
          ...
          correct: isCorrect,
          score: isCorrect
            ? Number(JSON.parse(quizRecord.Quiz.each_score).judge)
            : 0,
        };
        ...
        // 计算得分: 满分 * 答对空数量 / 总题数
        score += parseInt(
          (Number(JSON.parse(quizRecord.Quiz.each_score).fill) *
            correct_fill_count) /
            answers.length
        );
